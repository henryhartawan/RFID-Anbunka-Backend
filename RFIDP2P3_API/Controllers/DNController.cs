...
namespace RFIDP2P3_API.Controllers
{
    ...
    public class DNController : ControllerBase
    {
        ...
        [HttpPost]
        public ActionResult<IEnumerable<Dictionary<string,object>>> INQ(DN dn)
        {
            var dt = new DataTable();

			using (SqlConnection conn = new SqlConnection(_configuration))
			using (SqlCommand cmd = new SqlCommand("sp_Inq_T_DN", conn))
			{
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new("@Periode_ID", dn.Periode));
        ...
        {
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_Sync_To_EDN_DN", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
