...
namespace RFIDP2P3_API.Controllers
{
    ...
    [ApiController]
    public class ManualCallingController : ControllerBase
    {
        private readonly string _configuration;
        private string? remarks = "";

        public ManualCallingController(IConfiguration configuration)
        {
        ...
        public ActionResult<IEnumerable<Dictionary<string, object>>> INQ()
        {
            ...
            using (SqlCommand cmd = new SqlCommand("sp_Inq_T_TN_Manual_Calling", conn))
            {
        ...
        {
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_Submit_T_TN_Manual_Calling", conn))
            {
                conn.Open();
                cmd.CommandType = CommandType.StoredProcedure;
