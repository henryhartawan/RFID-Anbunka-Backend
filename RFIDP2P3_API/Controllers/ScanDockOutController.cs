...
namespace RFIDP2P3_API.Controllers
{
    ...
    [ApiController]
    public class ScanDockOutController : ControllerBase
    {
        private readonly string _configuration;
        private string? remarks = "";

        public ScanDockOutController(IConfiguration configuration)
        {
        ...
        public ActionResult<IEnumerable<Dictionary<string, object>>> INQ()
        {
            ...
            using (SqlCommand cmd = new SqlCommand("sp_Inq_T_TN_Scan_Dock_Out", conn))
            {
        ...
        {
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_Submit_T_TN_Scan_Dock_Out", conn))
            {
                conn.Open();
                cmd.CommandType = CommandType.StoredProcedure;
