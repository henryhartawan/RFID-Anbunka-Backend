...
namespace RFIDP2P3_API.Controllers
{
    ...
    [ApiController]
    public class ScanOutADMController : ControllerBase
    {
        private readonly string _configuration;
        private string? remarks = "";

        public ScanOutADMController(IConfiguration configuration)
        {
        ...
        public ActionResult<IEnumerable<Dictionary<string, object>>> INQ()
        {
            ...
            using (SqlCommand cmd = new SqlCommand("sp_Inq_T_TN_Scan_Out_ADM", conn))
            {
        ...
        {
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_Submit_T_TN_Scan_Out_ADM", conn))
            {
                conn.Open();
                cmd.CommandType = CommandType.StoredProcedure;
