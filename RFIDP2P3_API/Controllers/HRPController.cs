...
namespace RFIDP2P3_API.Controllers
{
    ...
    [ApiController]
    public class HRPController : ControllerBase
    {
        private readonly string _configuration;
        private string? remarks = "";

        public HRPController(IConfiguration configuration)
        {
            _configuration = configuration.GetConnectionString("DefaultConnection");
        }

        [HttpPost]
        public ActionResult<IEnumerable<Dictionary<string,object>>> INQ(HRP hrp)
        {
            var dt = new DataTable();

			using (SqlConnection conn = new SqlConnection(_configuration))
			using (SqlCommand cmd = new SqlCommand("sp_Inq_T_HRP", conn))
			{
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new("@Periode", hrp.Periode));
                conn.Open();
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    else
                    {
                        ...
                        {
                            conn.Open();
                            SqlCommand cmd = new("exec sp_Upload_T_HRP @EntryUser", conn);
