...
namespace RFIDP2P3_API.Controllers
{
	...
	[ApiController]
	public class MasterCILPFutureController : Controller
	{
		private readonly string _configuration;
		private string? remarks = "";

		public MasterCILPFutureController(IConfiguration configuration)
		{
			_configuration = configuration.GetConnectionString("DefaultConnection");
		}

		[HttpPost]
		public ActionResult<IEnumerable<MasterCILPFuture>> INQ()
		{
			List<MasterCILPFuture> cil = new();

			using (SqlConnection conn = new SqlConnection(_configuration))
			using (SqlCommand cmd = new SqlCommand("sp_M_CI_LP_Future_Sel", conn))
			{
		...
		[HttpPost]
		public ActionResult<IEnumerable<MasterCILPFuture>> INS(MasterCILPFuture cil)
		{
			using (SqlConnection conn = new SqlConnection(_configuration))
			using (SqlCommand cmd = new SqlCommand("sp_M_CI_LP_Future_Ins", conn))
			{
				cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.Parameters.Add(new("@Periode_ID", cil.Periode_ID));
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    else
                    {
                        ...
                        {
                            conn.Open();
                            SqlCommand cmd = new("exec sp_M_CI_LP_Future_Upload @EntryUser", conn);
        ...
        [HttpPost]
        public ActionResult<IEnumerable<MasterCILPFuture>> CheckTime(MasterCILPFuture cil)
        {
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_M_CI_LP_Future_Check", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@Remarks", SqlDbType.VarChar, 100).Direction = ParameterDirection.Output;
