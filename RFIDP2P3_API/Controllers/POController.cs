...
namespace RFIDP2P3_API.Controllers
{
    ...
    public class POController : ControllerBase
    {
        ...
        [HttpPost]
        public ActionResult<IEnumerable<Dictionary<string,object>>> INQ(PO po)
        {
            var dt = new DataTable();

			using (SqlConnection conn = new SqlConnection(_configuration))
			using (SqlCommand cmd = new SqlCommand("sp_Inq_T_PO", conn))
			{
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new("@Periode", po.Periode));
                conn.Open();
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    else
                    {
                        ...
                        {
                            conn.Open();
                            SqlCommand cmd = new("exec sp_Upload_T_PO @EntryUser", conn);
        ...
        [HttpPost]
        public ActionResult<IEnumerable<PO>> DEL(PO po)
        {
            using (SqlConnection conn = new SqlConnection(_configuration))
            using (SqlCommand cmd = new SqlCommand("sp_Delete_T_PO", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@Remarks", SqlDbType.VarChar, 100).Direction = ParameterDirection.Output;

                cmd.Parameters.Add(new("@Upload_ID", po.Upload_ID));
